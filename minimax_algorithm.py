...
def minimax(game_array, level, nr):
    ...
    if (level == "maxi"):
        ...
        for i in contestant_steppable:
            ...
            game_array[bot_pos[0]][bot_pos[1]] = 0  # the contestant's original position becomes zero and we select the next one
...
def minimax_heuristic(game_array, level, nr, alfa, beta):
    ...
    if (level == "maxi"):
        ...
        for i in bot_steppable:
            ...
            for j in contestant_blockable:
                ...
                if (val<result):
                    ...
                    if (nr==0 and alfa >= 1000):
                        ...
                        game_array[bot_pos[0]][bot_pos[1]] = 3
                        return (i[0],i[1]), (j[0],j[1])
                    step_and_block = replace(step_and_block, step_x = i[0], step_y = i[1], block_x = j[0], block_y = j[1])
            ...
            game_array[bot_pos[0]][bot_pos[1]] = 3
        if (nr==0 and val >= 1):
            return (step_and_block.step_x, step_and_block.step_y), (step_and_block.block_x, step_and_block.block_y)
        else:
            if (nr==0):
                return (i[0],i[1]),(j[0],j[1])
        return val
