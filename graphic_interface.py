    ...
    for i in range(0, tile_nr):
        for j in range(0, tile_nr):
            tile_list[i][j] = Button(
                ...
                width=120 // tile_nr,
                height=45 // tile_nr,
                command=lambda i=i, j=j: game_running(i, j),
            )
...
def game_running(i, j):
    ...
    if turn.get() == 0:
        if (
            game_array[i][j] == 0
            and abs(i - contestant_pos[0]) <= 1
            and abs(j - contestant_pos[1]) <= 1
        ):
...
def graphic_bot_move(tile_nr):
    ...
    if tile_nr > 3:
        optimal_pos = minimax_algorithm.minimax_heuristic(
            game_array_copy, "maxi", 0, -100, 100
        )
    else:
    ...
    tile_list[blocking_pos[0]][blocking_pos[1]].config(
...
def interface_menu():
    ...
    button_mode3 = Button(
        master, text="3x3", width=20, height=10, command=lambda: gameplay_initialiser(3)
    )
    button_mode5 = Button(
        master, text="5x5", width=20, height=10, command=lambda: gameplay_initialiser(5)
    )
    who_starts = Checkbutton(
        master,
        text="The bot should start",
        variable=turn,
        onvalue=2,
        offvalue=0,
        indicatoron=True,
